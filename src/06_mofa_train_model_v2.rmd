---
title: "Train MOFA model on CPTAC-PDAC RNA, proteome, and mutation data"
...
print(dim(rna_df))
cat("RNA missing values:", sum(is.na(rna_df)), "\n")
cat("RNA infinite values:", sum(is.infinite(as.matrix(rna_df))), "\n")

# load proteome data
prot_df <- read.csv("Data/MOFA_input/proteome_top5k_hvgs_scaled_tumor.csv", row.names = 1)
print(dim(prot_df))
cat("Proteome missing values:", sum(is.na(prot_df)), "\n")
cat("Proteome infinite values:", sum(is.infinite(as.matrix(prot_df))), "\n")

# load mutation data
mut_df <- read.csv("Data/MOFA_input/mutation_binary_min_1_mut_in_2caseIDs_tumor.csv", row.names = 1)
print(dim(mut_df))
cat("Mutation missing values:", sum(is.na(mut_df)), "\n")
cat("Mutation infinite values:", sum(is.infinite(as.matrix(mut_df))), "\n")
...
mofa_input <- list(RNA = as.matrix(as.data.frame(rna_df)),
                   Mutation = as.matrix(as.data.frame(mut_df)),
                   Protein = as.matrix(as.data.frame(prot_df))
)
...
model_opts$likelihoods <- c("gaussian", "bernoulli", "gaussian")  # Set likelihoods for RNA, Mutation, and Protein views
...
out_file <- "Results/MOFA_models/mofa_cptac_pdac_rna_proteome_mutations_model.hdf5"
...
if(sum(valid_elbo) > 1) {
  ...
  p <- ggplot(elbo_df, aes(x = iteration, y = elbo)) +
    ...
    labs(title = "MOFA Model Convergence (RNA + Mutation + Proteome)",
