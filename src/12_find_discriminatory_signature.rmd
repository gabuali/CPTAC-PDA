...
```{r factor2_scores}
# Load MOFA model
mofa_model <- load_model(here("Results", "MOFA_models", "mofa_cptac_pdac_rna_proteome_model.hdf5"))
...
clinical_stage_df <- read.csv(here("Data", "Metadata", "metadata_merged_clinical_mol-phenotype_from_paper.csv"), 
                              stringsAsFactors = FALSE)
...
write.csv(final_signature, 
          file = here("Results", "MOFA_Factor2_Protein_LassoSignature_18Proteins.csv"), 
          row.names = FALSE)
```